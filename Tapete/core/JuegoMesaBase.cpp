...
namespace tapete
{
    ...
    void JuegoMesaBase::posactualiza(double)
    {
        if (estado_intro_ != EstadoIntro::Fin)
        {
            ...
            if (estado_intro_ == EstadoIntro::Fin)
            {
                ...
                musica_intro_->para();
                delete musica_intro_;
                musica_intro_ = nullptr;
                for (auto *img : imgs_)
                    delete img;
                imgs_.clear();
    ...
    void JuegoMesaBase::agregaPersonaje(ActorPersonaje *e)
    {
        ...
        if (e->ladoTablero() == LadoTablero::Izquierda)
            personajes_izqrd_.push_back(e);
        else
            personajes_derch_.push_back(e);
    }
    ...
    void JuegoMesaBase::configura(SucesosJuegoComun *s, ModoJuegoBase *m)
    {
        sucesos_ = s;
        modo_ = m;
    }
