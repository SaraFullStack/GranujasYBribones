...
namespace tapete
{
    ...
    ModoJuegoComun::ModoJuegoComun(JuegoMesaBase *juego, EstadoJuegoComun &estado, EstadoJuegoComun &estado_previo) : estado_{estado},
    ...
    void ModoJuegoComun::saltaFinalJuego()
    {
        try
        {
            validaEstado(false, {EstadoJuegoComun::inicial,
    ...
    void ModoJuegoComun::entraAccionDesplazam()
    {
        try
        {
            validaEstado({EstadoJuegoComun::inicioJugada,
    ...
    void ModoJuegoComun::evaluaHabilidadOponente(int indice_habilidad)
    {
        try
        {
            ...
            if (!valido)
            { // la habilidad no se puede usar con el oponente
    ...
    void ModoJuegoComun::evaluaHabilidadOponente(ActorPersonaje *personaje)
    {
        try
        {
            validaEstado({EstadoJuegoComun::preparacionHabilidadOponente,
    ...
    void ModoJuegoComun::evaluaHabilidadArea(int indice_habilidad)
    {
        try
        {
            ...
            if (!valido)
            { // la habilidad no se puede usar con el área indicada
    ...
    void ModoJuegoComun::evaluaHabilidadArea(Coord celda)
    {
        try
        {
            validaEstado({EstadoJuegoComun::preparacionHabilidadArea,
        ...
        SimboloValida simbolo_area_celdas)
    {
        ...
        case NINGUNO:
            aserta(personajeElegido(LadoTablero::Izquierda) == nullptr &&
                   ...
                   "personajeElegido", simbolo_elegidos);
            break;
        case UNO:
            aserta((personajeElegido(LadoTablero::Izquierda) != nullptr &&
                    personajeElegido(LadoTablero::Derecha) == nullptr) ||
    ...
    void ModoJuegoComun::escribeEstado()
    {
        ...
        case EstadoJuegoComun::inicioJugada:
            juego()->tablero()->escribeMonitor(std::vector<string>{"Selecciona el retrato para mover la",
                                                                   "ficha o selecciona una habilidad."},
                                               {});
            break;
        case EstadoJuegoComun::preparacionDesplazamiento:
            juego()->tablero()->escribeMonitor(std::vector<string>{"Marca el camino comenzando en la",
                                                                   "ficha del personaje."},
                                               {});
            break;
        case EstadoJuegoComun::marcacionCaminoFicha:
            juego()->tablero()->escribeMonitor(std::vector<string>{"Marca las etapas del camino. Despues",
                                                                   "pulsa 'espacio' para mover la ficha",
                                                                   "o 'Esc' para cancelar."},
                                               {});
            break;
        case EstadoJuegoComun::habilidadSimpleInvalida:
            juego()->tablero()->escribeMonitor(std::vector<wstring>{L"La habilidad no se puede usar."},
                                               {habilidadAccion()->nombre()});
            break;
        case EstadoJuegoComun::habilidadSimpleConfirmacion:
            juego()->tablero()->escribeMonitor(std::vector<wstring>{L"Pulsa 'espacio' para usar la",
                                                                    L"habilidad, 'Esc' para cancelar."},
                                               {habilidadAccion()->nombre()});
            break;

        case EstadoJuegoComun::habilidadSimpleResultado:
            juego()->tablero()->escribeMonitor(std::vector<wstring>{L"Habilidad usada.",
                                                                    L"Pulsa 'espacio'.",
        ...
        case EstadoJuegoComun::preparacionHabilidadOponente:
            if (habilidadAccion()->antagonista() == Antagonista::aliado)
            {
                juego()->tablero()->escribeMonitor(std::vector<wstring>{L"Selecciona un aliado."},
                                                   {habilidadAccion()->nombre()});
            }
            else
            {
                juego()->tablero()->escribeMonitor(std::vector<wstring>{L"Selecciona un oponente."},
                                                   {habilidadAccion()->nombre()});
            }
            break;
        case EstadoJuegoComun::oponenteHabilidadInvalido:
            if (habilidadAccion()->antagonista() == Antagonista::aliado)
            {
                juego()->tablero()->escribeMonitor(std::vector<wstring>{L"La habilidad no se puede usar con",
                                                                        L"ese aliado."},
            ...
            {
                juego()->tablero()->escribeMonitor(std::vector<wstring>{L"La habilidad no se puede usar contra",
                                                                        L"ese oponente."},
                                                   {std::format(L"{} contra {}",
                                                                atacante()->nombre(),
                                                                oponente()->nombre()),
        ...
        case EstadoJuegoComun::oponenteHabilidadCalculando:
            juego()->tablero()->escribeMonitor(std::vector<wstring>{L"Calculando.",
                                                                    L"Espera."},
                                               {habilidadAccion()->nombre()});
            break;
        ...
        case EstadoJuegoComun::preparacionHabilidadArea:
            juego()->tablero()->escribeMonitor(std::vector<wstring>{L"Selecciona una celda para usar la",
                                                                    L"habilidad sobre un área."},
                                               {habilidadAccion()->nombre()});
            break;
        case EstadoJuegoComun::areaHabilidadInvalida:
            juego()->tablero()->escribeMonitor(std::vector<wstring>{L"La habilidad no se puede usar sobre",
                                                                    L"esas celdas."},
                                               {habilidadAccion()->nombre()});
            break;
        ...
        case EstadoJuegoComun::mostrandoAyuda:
            juego()->tablero()->escribeMonitor(std::vector<wstring>{L"Pulsa la botella para cerrar ",
                                                                    L"la ayuda."},
                                               {});
            break;
        case EstadoJuegoComun::finalPartida:
        {
            ...
            if (izquierdaDerrotada)
                mensaje = {L"¡Bribones ganan", L"Gracias por jugar!"};
            else if (derechaDerrotada)
                mensaje = {L"¡Granujas ganan", L"Gracias por jugar!"};
            else
