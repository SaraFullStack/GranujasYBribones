...
namespace tapete {
    ...
    const string & ActorTablero::archivoSonidoFracaso () {
        return archivo_sonido_fracaso;
    }

    int ActorTablero::volumenSonidoFracaso () {
        return volumen_sonido_fracaso;
    }

    void ActorTablero::asignaSonidoFracaso (const string & archivo, int volumen) {
        archivo_sonido_fracaso = archivo;
        volumen_sonido_fracaso = volumen;
    }

    void ActorTablero::emiteSonidoFracaso() {
        presencia_tablero.sonido_fracaso->suena();
    }

    const string & ActorTablero::archivoSonidoExito () {
        return archivo_sonido_exito;
    }

    int ActorTablero::volumenSonidoExito () {
        return volumen_sonido_exito;
    }

    void ActorTablero::asignaSonidoExito (const string & archivo, int volumen) {
        archivo_sonido_exito = archivo;
        volumen_sonido_exito = volumen;
    }

    void ActorTablero::emiteSonidoExito() {
        presencia_tablero.sonido_exito->suena();
    }
...
//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    void ActorTablero::actualiza (double tiempo_seg) {
        //TODO: POSICIONAR MAPA
        // Vector mouse = unir2d::Raton::posicion();
        // Vector relativo = mouse - PresenciaTablero::regionBandaAbajo.posicion();

        // std::cout << "Relativo a banda abajo: x=" << relativo.x() << ", y=" << relativo.y() << std::endl;
       
        //TODO: POSICIONAR OBJETOS REJILLA
        // Vector mouse = unir2d::Raton::posicion();
        // Vector relativo = mouse - PresenciaTablero::regionRejilla.posicion();  // <- clave

        // Coord celda;
        // rejilla_tablero.localizaCelda(relativo, celda);

        // std::cout << "Fila: " << celda.fila() << ", Columna: " << celda.coln() << std::endl;
